...
def ssw_xargs_parser(input_dir, TMPDIR):
    d = {}
    ...
    with open(f"{TMPDIR}/{name}/ssw_xargs/{name}.ssw_xargs.json", "w") as f:
        print(json.dumps(d), file=f)


def output_parser(input_dir, TMPDIR):
    name = input_dir.split('/')[-2]
    f_out = open(f"{TMPDIR}/{name}/ssw_xargs/{name}.ssw_xargs.results", "w")
    for input_file in os.scandir(f"{input_dir}"):
        if os.path.splitext(input_file)[1] == ".ssw_xargs":
            with open(input_file) as f:
                query, db, score = None, None, None
