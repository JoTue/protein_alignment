...
def adept_as_xargs(query_file, db_file, name, matrix, gapopen, gapextension, min_score, num_gpus, chunk_s):
    ...
    if chunk_s:
        ...
        subprocess.run(f"seq {chunk_n} | xargs -I[] --max-procs={threads_n} bash -c 'let \"i=([]-1)%{num_gpus}\"; echo start [] $i; date; uptime; nvidia-smi; ../adept/build2/examples/py_examples/py_asynch_protein5 -q {os.path.dirname(query_file)}/{chunk_name}/q_[] -r {db_file} -o ../program_out/{name}/adept_as_xargs/[].adept_as_xargs -m {matrix} --gapopen {gapopen} -e {gapextension} -c {min_score} -g $i --max_query {max_query} --max_ref {max_ref} ; echo end [] $i; date; uptime; nvidia-smi'", shell=True)
    else:
        subprocess.run(f"seq {chunk_n} | xargs -I[] --max-procs={threads_n} bash -c 'let \"i=[]-1\"; echo start []; date; uptime; nvidia-smi; ../adept/build2/examples/py_examples/py_asynch_protein5 -q {os.path.dirname(query_file)}/{chunk_name}/q_[] -r {db_file} -o ../program_out/{name}/adept_as_xargs/[].adept_as_xargs -m {matrix} --gapopen {gapopen} -e {gapextension} -c {min_score} -g $i --max_query {max_query} --max_ref {max_ref} ; echo end []; date; uptime; nvidia-smi'", shell=True)
...
def main():
    ...
    adept_as_xargs(query_file, db_file, name, args.matrix, args.gapopen, args.gapextension, args.min_score, args.num_gpus, args.chunksize)
