...
def swipe(input_file, matrix, gapopen, gapextension, num_threads, min_score):
    ...
    subprocess.run(f"swipe -i {input_file} -d {input_file} -G {gapopen-gapextension} -E {gapextension} -M /apps/emboss/6.6.0/emboss/data/E{matrix} -a {num_threads} -e 1000000000 -b 1000000000 -v 1000000000 -c {min_score} > ../program_out/{name}/swipe/{name}.swipe", shell=True)
...
def main():
    ...
    swipe(args.input, args.matrix, args.gapopen, args.gapextension, args.num_threads, args.min_score)
